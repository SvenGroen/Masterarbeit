...
\begin{description}
	\item[R1 - Functional Suitability:]
		...
		In the context of this thesis, the system needs to be able to show how different tabular processing techniques influence the performance of an existing data synthesis approach.
...
\begin{description}
	...
	\item[eval\_seeds.py:]
		...
		Starts by loading the pre-trained sampling model \\([TabDDPM|SMOTE|CTABGAN|CTABGAN+|TVAE]).
...
The variable \textit{model\_type} refers to the model that should approximate the noise in the reverse noising process.
